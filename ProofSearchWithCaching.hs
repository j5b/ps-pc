...
type CachedEntry = Either (Model, [Individual]) ProofTree
type ExpandingEntry = String -> Individual -> Model
type Cache = [([Concept], Either CachedEntry ExpandingEntry)]
...
findProofOrModel cs gamma (i:is) cache
  = maybe (result, cache') cacheLookup (lookup cs cache)
    where
      expanding = (cs, Right (\ rel j -> ([],[],[(rel, [(j,i)])])))
      result = findProofOrModel' cs gamma (i:is) (expanding:cache)
      cache' = (cs, Left result):cache

      cacheLookup :: Either CachedEntry ExpandingEntry
                     -> (Either (Model, [Individual]) ProofTree, Cache)
      cacheLookup (Left pom)   = (pom, cache)
      cacheLookup (Right loop) = (Left ((loop "R" (i-1)), i:is), cache) -- for now
...
foldExists cs gamma (Exists rel c : es) (i:is) cache
  ...
  where
    (pom, cache')
        = findProofOrModel (applyExists cs gamma (Exists rel c)) gamma is cache
    ...
    dealWithModel (m, is') = either (\(m', is'') -> Left (joinModels m' m'', is''))
      ...
      where
        m'' = joinModels m $ joinModels backedgemodel
